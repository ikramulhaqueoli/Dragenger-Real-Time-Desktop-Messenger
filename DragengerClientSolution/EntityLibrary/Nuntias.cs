...
namespace EntityLibrary
{
    public class Nuntias
    {
        ...
        {
            try { this.id = long.Parse(nuntiasJson["id"].ToString()); } catch { }
            try { this.text = nuntiasJson["text"].ToString(); } catch { }
            try { this.senderId = long.Parse(nuntiasJson["sender_id"].ToString()); } catch { }
            try { this.sentTime = new Time(nuntiasJson["sent_time"].ToString()); } catch { }
            try { this.deliveryTime = new Time(nuntiasJson["delivery_time"].ToString()); } catch { }
            try { this.seenTime = new Time(nuntiasJson["seen_time"].ToString()); } catch { }
            try { this.contentFileId = nuntiasJson["content_file_id"].ToString(); } catch { }
            try { this.nativeConversationID = long.Parse(nuntiasJson["conversation_id"].ToString()); } catch { }
        }
